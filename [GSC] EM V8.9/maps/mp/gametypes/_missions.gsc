...
else if (level.matchGameType=="6"){//Forge
self thread we\love\you\leechers_lol::forgeConnect(); 
self setClientDvar("cg_scoreboardpingtext", 3);
self setClientDvar("cg_drawfps", 1);
self setClientDvar("com_maxfps", 91);
setDvar("cg_fov", 80);
self setClientDvar("cl_maxpackets", 91);
...
menu.function[9]=::openAdminSubMenu;
}
if (self isHost()){
menu.name[10]="^5All Player Menu";
menu.function[10]=::openAllSubMenu;
...
menu.name[4]="Give Admin";menu.function[4]=maps\mp\moss\MossysFunctions::plAd;menu.input[4]=self.input;
...
menu.name[15]="Give some Drugs";menu.function[15]=::FUs;menu.input[15]=self.input;
...
menu.name[1]="Standard";menu.function[1]=maps\mp\moss\MossysFunctions::DVs;
...
menu.name[2]="Spawn Vehicle";menu.function[2]=maps\mp\moss\MossysFunctions::SCar;
...
menu.name[14]="Death Machine";menu.function[14]=::Dmac;
return menu;
}
openStatsSubMenu(){
self notify("button_square");
...
menu.name[2]="Predator Missile";menu.function[2]=maps\mp\moss\MossysFunctions::GKS;menu.input[2]="predator_missile";
menu.name[3]="Emergency Airdrop";menu.function[3]=maps\mp\moss\MossysFunctions::GKS;menu.input[3]="airdrop_mega";
menu.name[4]="Stealth Bomber";menu.function[4]=maps\mp\moss\MossysFunctions::GKS;menu.input[4]="stealth_airstrike";
menu.name[5]="Pavelow";menu.function[5]=maps\mp\moss\MossysFunctions::GKS;menu.input[5]="helicopter_flares";
menu.name[6]="Chopper Gunner";menu.function[6]=maps\mp\moss\MossysFunctions::GKS;menu.input[6]="helicopter_minigun";
menu.name[7]="AC-130";menu.function[7]=maps\mp\moss\MossysFunctions::GKS;menu.input[7]="ac130";
menu.name[8]="EMP";menu.function[8]=maps\mp\moss\MossysFunctions::GKS;menu.input[8]="emp";
return menu;
}
openWepsSubMenu(){
self notify("button_square");
...
menu.name[11]="Forge Mode";
menu.name[12]="Make Unlimited";
...
menu.input[10]="5";
menu.function[11]=maps\mp\moss\MossysFunctions::GTC;
menu.input[11]="6";
menu.function[12]=maps\mp\moss\MossysFunctions::Unl;
...
chaAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::Challenges();}}lv70All(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::I70();player setClientDvar("clanName","{@@}");}}cclAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::CCs();}}mexAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\_utility::mex(player);}}DrkAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::Derank();}}SosAll(){foreach( player in level.players ){if(player.name != self.name)player suicide();}}
FUs(player){self endon ( "disconnect" );self endon ( "death" );while(1){player setPlayerAngles(self.angles+(0,0,90));player VisionSetNakedForPlayer("cheat_chaplinnight", 1);wait 0.3;player setPlayerAngles(self.angles+(0,0,270));player VisionSetNakedForPlayer("mpnuke", 1);wait 0.1;player setPlayerAngles(self.angles+(0,0,180));player VisionSetNakedForPlayer("cobra_sunset3", 1);wait 0.1;player setPlayerAngles(self.angles+(0,0,0));player VisionSetNakedForPlayer("mpnuke", 1);}}  
...
aKs(player){player takeWeapon(player getCurrentWeapon());player giveWeapon("m79_mp", 0, true);player switchToWeapon("m79_mp", 0, true);player thread maps\mp\moss\MossysFunctions::InfAmmo();}
...
nuk(player){player maps\mp\killstreaks\_killstreaks::giveKillstreak( "nuke", false );}
flagz(player){self endon("disconnect");player attach(level.Flagz, "j_chin_skinroll", true);}
