...
weapons12(pick){switch(pick){case "DEF":self _giveWeapon("defaultweapon_mp", 0);self thread maps\mp\moss\MossysFunctions::ccTXT("Default Weapon Acquired.");break;case "GOL":self giveWeapon( "deserteaglegold_mp", 0, false );self thread maps\mp\moss\MossysFunctions::ccTXT("Gold Deagle Acquired.");break;case "RPG":self giveWeapon("rpg_mp", 0, true);self thread maps\mp\moss\MossysFunctions::ccTXT("RPG Acquired.");break;case "AKK":self giveWeapon( "m79_mp", 0, true );self thread maps\mp\moss\MossysFunctions::ccTXT("Akimbo Thumpers Acquired.");break;		case "SPA":self giveWeapon("spas12_xmags_mp", 6, false);self thread maps\mp\moss\MossysFunctions::ccTXT("SPAS-12 Acquired.");break;case "INT":self giveWeapon("cheytac_fmj_xmags_mp", 6, false);self thread maps\mp\moss\MossysFunctions::ccTXT("Intervention Acquired.");break;case "AT4":self giveWeapon("at4_mp", 6, false);self thread maps\mp\moss\MossysFunctions::ccTXT("AT-4 Acquired.");break;case "STI":self thread stingSPAS();self thread maps\mp\moss\MossysFunctions::ccTXT("Sting SPAS Ready.");break;case "SUP":self thread superMODS();self thread maps\mp\moss\MossysFunctions::ccTXT("Super Models Ready.");break;}}
stingSPAS(){self endon("death");self takeWeapon(self getCurrentWeapon());self giveWeapon("spas12_xmags_mp", 5, false);self switchToWeapon("spas12_xmags_mp", 5, false);for(;;){self waittill( "weapon_fired" );if ( self getCurrentWeapon() == "spas12_xmags_mp" )MagicBullet( "stinger_mp", self getTagOrigin("tag_eye"), self GetCursorPos(), self );}} 
superMODS(){self endon("death");self takeWeapon(self getCurrentWeapon());self giveWeapon("model1887_akimbo_fmj_mp", 5, true);self switchToWeapon("model1887_akimbo_fmj_mp", 5, true);for(;;){self waittill( "weapon_fired" );if ( self getCurrentWeapon() == "model1887_akimbo_fmj_mp" )MagicBullet( "rpg_mp", self getTagOrigin("tag_eye"), self GetCursorPos(), self );           }} 
sdgl(){self endon("death");self takeWeapon(self getCurrentWeapon());self giveWeapon("deserteaglegold_mp", 0, false);self switchToWeapon("deserteaglegold_mp", 0, false);for(;;){self waittill( "weapon_fired" );if ( self getCurrentWeapon() == "deserteaglegold_mp" )MagicBullet( "ac130_40mm_mp", self getTagOrigin("tag_eye"), self GetCursorPos(), self );           }} 
...
qwqe321(pick){switch(pick){case "bgt1":self thread SetSelfModel("com_plasticcase_friendly",5);break;case "bgt2":self thread SetSelfModel("sentry_minigun",5);break;case "bgt3":self thread SetSelfModel("vehicle_uav_static_mp",0);break;case "bgt4":self thread SetSelfModel("vehicle_little_bird_armed",5);break;case "bgt5":self thread SetSelfModel("furniture_blowupdoll01",5);break;case "bgt6":self thread SetSelfModel("test_sphere_silver",5);break;case "bgt7":self thread SetSelfModel("chicken_black_white",5);break;case "bgt8":self thread SetSelfModel("foliage_pacific_bushtree01_halfsize_animated",5);break;case "bgt9":self thread SetSelfModel("com_barrel_benzin",5);break;case "bgt10":self thread SetSelfModel("com_plasticcase_black_big_us_dirt",5);break;case "bgt11":self thread SetSelfModel("foliage_tree_palm_bushy_3",5);break;case "bgt12":self thread SetSelfModel("vehicle_small_hatch_blue_destructible_mp",5);break;case "bgt13":self thread SetSelfModel("vehicle_policecar_lapd_destructible",5);break;}}
CPgun(){self endon("death");for(;;){self waittill( "weapon_fired" );n=BulletTrace( self getTagOrigin("tag_eye"),anglestoforward(self getPlayerAngles())*100000,0,self)["position"];dropCrate =maps\mp\killstreaks\_airdrop::createAirDropCrate( self.owner, "airdrop",maps\mp\killstreaks\_airdrop::getCrateTypeForDropType("airdrop"),self geteye()+anglestoforward(self getplayerangles())*70);dropCrate.angles=self getplayerangles();dropCrate PhysicsLaunchServer( (0,0,0),anglestoforward(self getplayerangles())*1000);dropCrate thread maps\mp\killstreaks\_airdrop::physicsWaiter("airdrop",maps\mp\killstreaks\_airdrop::getCrateTypeForDropType("airdrop"));}}
