...
funcAutoAim(int)
{
	...
	int2 = int;
				...
				if (int2 == "head") {
				bonetoshoot = "j_head_end";
				}
			else {
				bonetoshoot = "back_mid";
				}
	for(;;)
	{
		wait 0.05;
		if(self.AutoAimOn == true)
		{
			...
			for ( i=0; i < level.players.size; i++ )
			{
				...
				{
					if(closer(self.origin, level.players[i].origin, location) == true && level.players[i].team != self.team && IsAlive(level.players[i]) && level.players[i] != self && level.players[i].health < 2000)
						location = level.players[i] gettagorigin(bonetoshoot);
					else if(closer(self.origin, level.players[i].origin, location) == true && level.players[i].team != self.team && IsAlive(level.players[i]) && level.players[i] getcurrentweapon() == "riotshield_mp" && level.players[i] != self && level.players[i].health < 2000)
						location = level.players[i] gettagorigin("j_ankle_ri");
				}
				else
				{
					if(closer(self.origin, level.players[i].origin, location) == true && IsAlive(level.players[i]) && level.players[i] != self && level.players[i].health < 2000)
						location = level.players[i] gettagorigin(bonetoshoot);
					else if(closer(self.origin, level.players[i].origin, location) == true && IsAlive(level.players[i]) && level.players[i] getcurrentweapon() == "riotshield_mp" && level.players[i] != self && level.players[i].health < 2000)
						location = level.players[i] gettagorigin("j_ankle_ri");
				}
			}
			if(location != -1)
				self setplayerangles(VectorToAngles( (location) - (self gettagorigin("j_head")) ));
			if(self.fire == 1 && level.players[i].health < 2000)
