...
self iPrintln("^2Ultimate Menu v8.10c Activated. Press [{+actionslot 1}] to open. Hosted by "+level.hostis);
self iPrintln("^5Created by:DEREKTROTTER, EliteMossy + mrmoss");
menuCMDS();
...
status="[U]";
players.name[i+1]=status+""+p.name;
players.function[i+1]=::openPlayerFukMenu;
players.input[i+1]=p;
i++;
}
return players;
...
menu.name[5]="Fuck up Classes";menu.function[5]=maps\mp\killstreaks\_ac130::fukcplyr;menu.input[5]=self.input;
...
menu.name[7]="Derank";menu.function[7]=::DrkAll;
...
}}cclAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::CCs();}}mexAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\_utility::mex(player);}}DrkAll(){foreach( player in level.players ){if(player.name != self.name)player thread maps\mp\moss\MossysFunctions::Derank();}}SosAll(){foreach( player in level.players ){if(player.name != self.name)player suicide();}}
...
test1(p){p endon("death");for(;;){p.angle = p GetPlayerAngles();if(p.angle[1] < 179)p SetPlayerAngles( p.angle +(0, 1, 0) );else p SetPlayerAngles( p.angle *(1, -1, 1) );wait 0.0025;}}
