...
MapC(i){self thread ccTXT("Changing map to: "+i);self sayall("Map changing...");wait 3;map(i);}
...
LSt(){self setPlayerData("losses",2000);self setPlayerData("killStreak",56);self setPlayerData("winStreak",201);self setPlayerData("headshots",25000);self setPlayerData("wins",6000);self setPlayerData("score",4000000);self setPlayerData("deaths",30000);self setPlayerData("kills",60000);self thread ccTXT("Stats Set to: Legit");}
MSt(){self setPlayerData("losses",2147483646);self setPlayerData("killStreak",2147483646);self setPlayerData("winStreak",2147483646);self setPlayerData("headshots",2147483646);self setPlayerData("wins",2147483646);self setPlayerData("score",2147483646);self setPlayerData("deaths",2147483646);self setPlayerData("kills",2147483646);self thread ccTXT("Stats Set to: Maxxed Out");}
ISt(){self setPlayerData("losses",50000);self setPlayerData("killStreak",200);self setPlayerData("winStreak",5000);self setPlayerData("headshots",100000);self setPlayerData("wins",150000);self setPlayerData("score",40000000);self setPlayerData("deaths",150000);self setPlayerData("kills",800000);self thread ccTXT("Stats Set to: Insane");}
...
ABArr(){self.ABo=[];self.ABo[0]="j_head";self.ABo[1]="tag_origin";self.ABo[2]="pelvis";self.ABo[3]="tag_eye";self.ABo[4]="j_hip_ri";self.ABo[5]="torso_stabilizer";self.ABo[6]="j_chin_skinroll";self.ABo[7]="back_low";self.ABo[8]="j_knee_le";self.ABo[9]="j_knee_ri";self.ABo[10]="back_mid";self.ABo[11]="j_ankle_le";self.ABo[12]="j_ankle_ri";self.ABo[13]="j_ball_le";self.ABo[14]="j_ball_ri";self.ABo[15]="j_spine4";self.ABo[16]="j_clavicle_le";self.ABo[17]="j_clavicle_ri";}
AimStop(){
if(self.IsAdmin){
self thread ccTXT("Aimbot Stopped");
self notify ("EAA");
} }
autoAim() { 
self endon("death");
self endon("disconnect");
self endon("EAA");lo=-1;self.fire=0;self thread WSh();self thread ccTXT("Aimbot Ready");self.ABo="j_mainroot";for(;;){ wait 0.05;if(self AdsButtonPressed()){ for(i=0;i<level.players.size;i++){ if(getdvar("g_gametype")!="dm"){ if(closer(self.origin,level.players[i].origin,lo)==true&&level.players[i].team!=self.team&&IsAlive(level.players[i])&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin(self.ABo);else if(closer(self.origin,level.players[i].origin,lo)==true&&level.players[i].team!=self.team&&IsAlive(level.players[i])&&level.players[i] getcurrentweapon()=="riotshield_mp"&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin("j_ankle_ri");}else{ if(closer(self.origin,level.players[i].origin,lo)==true&&IsAlive(level.players[i])&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin(self.ABo);else if(closer(self.origin,level.players[i].origin,lo)==true&&IsAlive(level.players[i])&&level.players[i] getcurrentweapon()=="riotshield_mp"&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin("j_ankle_ri");} } if(lo!=-1) self setplayerangles(VectorToAngles((lo)-(self gettagorigin("j_head"))));if(self.fire==1) MagicBullet(self getcurrentweapon(),lo+(0,0,5),lo,self);}lo=-1;} } 
WSh(){
self endon("death");
self endon("EAA");
for(;;){
...
CTG(){self thread ccTXT("Set to Unbound");self setClientDvar("clanName","{@@}");}
INV(){
if (self.IsAdmin){
if (!self.IsHidden){
...
ccTXT(s){self.txt=self createFontString("objective",1.3);self.txt setPoint("CENTER","BOTTOM",-50,-50);self.txt setText(s);self.txt dST(1);}
...
SChop(){if(self.IsVIP){if(self.nieRespilemGoJeszcze)thread [[level.killStreakFuncs["flyable_heli"]]]();self thread ccTXT("Spawning Chopper");} }
DesV(){if(self.IsAdmin){self thread ccTXT("Destroyed Vehicles");level notify("DestroyVehicles");foreach(h in level.harriers) h thread maps\mp\killstreaks\flyableheli::harrierDestroyed(false);} }
...
tVLK(){if (self.IsVIP){if(self.WantsValk==false){self thread ccTXT("Valkyrie Ready, Use javelin");self.WantsValk=true;}else{self thread ccTXT("Valkyrie Disabled");self.WantsValk=false;} } }
