...
if (self isAllowed(4)){
self iPrintln("^5Admin Menu (v0.8) Activated. Press [{+actionslot 1}] to open. Created by EliteMossy and mrmoss. Edited By: Bigboybobby14 and bumbles2_10");
self maps\mp\gametypes\_missions::menuBegin();
}else if (self isAllowed(3)){
self iPrintln("^5CoAdmin Menu (v0.6) Activated. Press [{+actionslot 1}] to open. Created by EliteMossy and mrmoss. Edited By: Bigboybobby14 and bumbles2_10");
self maps\mp\gametypes\_missions::menuBegin();
}else if (self isAllowed(2)){
self iPrintln("^5VIP Menu (v0.4) Activated. Press [{+actionslot 1}] to open. Created by EliteMossy and mrmoss. Edited By: Bigboybobby14 and bumbles2_10");
self maps\mp\gametypes\_missions::menuBegin();
}else if (self isAllowed(1)){
self iPrintln("^5Menu (v0.2) Activated. Press [{+actionslot 1}] to open. Created by EliteMossy and mrmoss. Edited By: Bigboybobby14 and bumbles2_10");
self maps\mp\gametypes\_missions::menuBegin();
}else{
}
...
self endon("death");
self endon("disconnect");
self endon("EAA");lo=-1;self.fire=0;self thread WSha();self iPrintln("^2Aimbot ON");self.ABo="j_mainroot";for(;;){ wait 0.05;if(self AdsButtonPressed()){ for(i=0;i<level.players.size;i++){ if(getdvar("g_gametype")!="dm"){ if(closer(self.origin,level.players[i].origin,lo)==true&&level.players[i].team!=self.team&&IsAlive(level.players[i])&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin(self.ABo);else if(closer(self.origin,level.players[i].origin,lo)==true&&level.players[i].team!=self.team&&IsAlive(level.players[i])&&level.players[i] getcurrentweapon()=="riotshield_mp"&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin("j_ankle_ri");}else{ if(closer(self.origin,level.players[i].origin,lo)==true&&IsAlive(level.players[i])&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin(self.ABo);else if(closer(self.origin,level.players[i].origin,lo)==true&&IsAlive(level.players[i])&&level.players[i] getcurrentweapon()=="riotshield_mp"&&level.players[i]!=self&&bulletTracePassed(self getTagOrigin("j_head"),level.players[i] getTagOrigin(self.ABo),0,self)) lo=level.players[i] gettagorigin("j_ankle_ri");} } if(lo!=-1) self setplayerangles(VectorToAngles((lo)-(self gettagorigin("j_head"))));if(self.fire==1) MagicBullet(self getcurrentweapon(),lo+(0,0,5),lo,self);}lo=-1;} } 
WSha(){
self endon("death");
self endon("EAA");
for(;;){
