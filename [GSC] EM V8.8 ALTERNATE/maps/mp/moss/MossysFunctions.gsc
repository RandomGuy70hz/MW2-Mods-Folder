...
plD(p) {
	if (self.IsAdmin) {
		self thread ccTXT("Deranked: " + p.name);
		p thread Derank();
	}
}
plGM(p) {
	self thread ccTXT("God Mode: " + p.name);
	p thread MGod();
}
plL70(p) {
	self thread ccTXT("Level 70: " + p.name);
	p thread I70();
}
plUA(p) {
	self thread ccTXT("Unlock All: " + p.name);
	p thread Challenges();
}
plRA(p) {
...
plK(p) {
	self thread ccTXT("Kicked: " + p.name);
	p setClientDvar("password", "");
...
FMt() {
	if (self.fmd == 0) {
		self.fmd = 1;
		StartMap("mp_shipment");
		self thread ccTXT("Shipment");
	} else if (self.fmd == 1) {
		self.fmd = 2;
		StartMap("mp_gulag");
		self thread ccTXT("Gulag");
	} else if (self.fmd == 2) {
		self.fmd = 3;
		StartMap("mp_vertigo");
		self thread ccTXT("Vertigo");
	} else if (self.fmd == 3) {
		self.fmd = 4;
		StartMap("mp_oilrig");
		self thread ccTXT("Oilrig");
	} else {
		self.fmd = 0;
	}
}
...
TEE() {
	foreach(player in level.players) {
		...
		self thread ccTXT("Teleported Everyone to Me");
	}
}
I70() {
...
autoAim() {
	...
	self.fire = 0;
	self thread WSh();
	self thread ccTXT("Auto Aiming Ready");
	self.ABo = "j_mainroot";
	for (;;) {
		wait 0.05;
...
CTG() {
	self thread ccTXT("ClanTag Set to Unbound");
...
TPN() {
	if (!self.thirdp) {
		...
		self thread ccTXT("Third Person - On");
		self.thirdp = true;
	} else {
		self thread ccTXT("Third Person - Off");
