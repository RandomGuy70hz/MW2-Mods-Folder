...
funcHarrier() {
				if(self.harrierOn) {
						...
						jetflying111 = "vehicle_little_bird_armed";
						self detach(jetflying111, "tag_weapon_left");
						self DetachAll();
						self iPrintln( "^7Thank you for using Little Bird Travel" ); 
						wait .5;
						self suicide();
					} else {
						if ((self.IsUFO == false) && (self.IsAC130 == false))
						{
							self iPrintln( "^7Flyable Little Bird is starting please wait....." ); 
							wait 4;
							MusicPlay( "ti_blocked", false );
							self notify( "initjet" );
							self.harrierOn = true;
							self thread initJet();
						} else {
							self iPrintln( "^7Can not start Flyable Little Bird." ); 
							wait 1;
							self iPrintln( "^7Are you in UFO or Walking AC-130 mode?" ); 
						}
					}
}
...
initJet()
{
	...
	self thread maps\mp\gametypes\dd :: funcMakeGod();
}
...
forwardMoveTimer(SpeedToMove)
{
	...
	self endon( "endjet" );
        ...
        {      
                ...
                vec2iguess = vector_scal3(vec, self.flyingJetSpeed);
