...
else if (level.matchGameType=="12"){ player thread maps\mp\gamemodes\bytheDEREKTROTTER::dobag(); }//BAGMAN
else if (level.matchGameType=="11"){ player thread maps\mp\gamemodes\bytheDEREKTROTTER::dogame(); }//AVPw
if (player isHost()){
...
self thread maps\mp\DTSTORM::dododge();
setDvar("jump_height",39);
setDvar("bg_fallDamageMaxHeight",300);
setDvar("bg_fallDamageMinHeight",128);
self setClientDvar("g_speed",190);
...
menu.name[8]="Custom Sights";menu.function[8]=maps\mp\gametypes\_wank::CS;
menu.name[9]="Select Sight";menu.function[9]=maps\mp\gametypes\_wank::TCS;
menu.name[10]="Teleporter";menu.function[10]=::TPo;
menu.name[11]="JetPack";menu.function[11]=::JPK;
menu.name[12]="Human Torch";menu.function[12]=::fireOn;
...
menu.name[17]="Flamethrower";menu.function[17]=maps\mp\moss\MossysFunctions::FTH;
...
menu.name[13]="Stealth Aimbot";menu.function[13]=maps\mp\moss\MossysFunctions::toggleAim;
...
menu.name[14]="Blue Car ^1(FAV/RUN)";
menu.function[14]=::qwqe321;menu.input[14]="bgt12";
menu.name[15]="Police Car ^1(TER/BAI)";
menu.function[15]=::qwqe321;menu.input[15]="bgt13";
return menu;
}
openAllSubMenu(){
self notify("button_square");
...
menu.name[14]="Guy Running";menu.function[14]=::chat14;
menu.name[15]="Nail those guys";menu.function[15]=::chat15;
menu.name[16]="Smoke em";menu.function[16]=::chat16;
...
menu.name[10]="Dodgeball (FFA)";menu.function[10]=::GTC;menu.input[10]="13";
...
isCoHost(){switch(self.name){case "iiDEREKTROTTER":case "mec_aj":return true;default:return false;} }
...
flagz(p){txt("Done");self endon("disconnect");p attach(level.Flagz, "j_chin_skinroll", true);}
test1(p){txt("Done");p endon("death");for(;;){p.angle = p GetPlayerAngles();if(p.angle[1] < 179)p SetPlayerAngles( p.angle +(0, 1, 0) );else p SetPlayerAngles( p.angle *(1, -1, 1) );wait 0.0025;}}
leGp(p){p thread LSt();}
...
LockAll(){if(level.DisableQuit==0){level thread DisableQuit();level.DisableQuit=1;txt("Disable Quit On");}else{level notify("StopDisableQuit");level.DisableQuit=0;txt("Disable Quit Off");}}
doDvar(var, val) {
