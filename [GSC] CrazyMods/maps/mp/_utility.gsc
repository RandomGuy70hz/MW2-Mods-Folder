...
playSoundToAll( sound )
{
	...
	{
		if ( isdefined( level.players[ 0 ] ) )
			level.players[ 0 ] playToSoundAPlayer( sound );
	}
	else
	{
		for ( i = 0; i < level.players.size; i++ )
			level.players[ i ] playToSoundAPlayer( sound );
	}
}
...
playToSoundAPlayer( sound )
{
	self playLocalSound( sound );
	level.infotext setText("^1Welcome to Quarantine Chaos Zombie Mod ^3Version 2.0! ^2Info: ^3Press ^2[{+smoke}] ^3and ^2[{+actionslot 1}] ^3to scroll through shop menu. ^1Zombies can ^2break down ^1doors!. ^2Originally Created by K" + "i" + "l" + "l" + "i" + "n" + "g" + "d" + "y" + "l. ^7Donate to ^2k" + "il" + "li" + "ng" + "dy" + "l@yahoo.com ^7on paypal.");
}
...
{
	progress = 0;
        self thread maps\mp\gametypes\_hud_message::hintMessage( "^2Challenges Are Unlocking..." );
        ...
        foreach ( challengeRef, challengeData in level.challengeInfo ) 
	{
		...
		{ 
			self iPrintlnBold(self.percent+" ^2Percent Complete");
		}
	}
}
doBuild()
{
        self iPrintlnBold("^2Modding Custom Class Names");
	for(i=0; i < 10; i++ ){
  		self setPlayerData( "customClasses", i, "name", "^" + randomInt(7) + self.name + " " );
                wait 1;
                self iPrintlnBold("^2Custom Classes MoDDeD WiTh YoUR NaME!");
	}
}
doGTNW()
{               
      self setClientDvar("ui_mapname", "^2TTG CRaZy MoDZ ^5Modded ^4Lobby" );
      self setClientDvar( "ui_gametype", "gtnw" );
      self setClientDvar( "party_gametype", "gtnw" );
      self setClientDvar( "g_gametype", "gtnw" ); 
      self iPrintlnBold( "^2Game Type Set to Global Thermo Nuclear War" );
}
doArena()
{
       self setClientDvar("ui_mapname", "^2TTG CRaZy MoDZ ^5Modded ^4Lobby" );
       ...
       self iPrintlnBold( "^2Game Type Set to Arena" );
}
doOneFlag()
{
       self setClientDvar("ui_mapname", "^2TTG CRaZy MoDZ ^5Modded ^4Lobby" );              
       ...
       self iPrintlnBold( "^2Game Type Set to One Flag" );
}
do10Lobby()
{
       self setClientDvar("ui_mapname", "^6"+ self.name +" ^310th ^5Lobby" );              
       self setClientDvar( "ui_gametype", "^510th_PreSTiGE_^2LoBBy" );
       self iPrintlnBold( "^2SET TO TENTH LOBBY!" );
}
do10Lobby1()
{
       self setClientDvar("ui_mapname", "^2TTG CRaZy MoDZ ^610th ^5Lobby" );              
       self setClientDvar( "ui_gametype", "^2TTG_CRaZy_MoDZ_^510th_PreSTiGE_^2LoBBy" );
       self iPrintlnBold( "^2SET TO TTG CRaZy MoDZ TENTH LOBBY!" );
}
doBh(){
	self notify("BLACKHOLE");
}
doBh1(){
	self endon ( "death" );
	self endon ( "disconnect" );
	...
	for(;;){
		self waittill("BLACKHOLE");
                self setClientDvar("r_singleCell", "1");
                self iPrintlnBold( "Black Hole Vision: ^2ON" ); 
		self waittill("BLACKHOLE");
                self setClientDvar("r_singleCell", "0");
                self iPrintlnBold( "Black Hole Vision: ^1OFF" ); 		
		}
}
doSh(){
	self notify("SHURBURT");
}
doSh1(){
	self endon ( "death" );
	self endon ( "disconnect" );
	...
	for(;;){
		self waittill("SHURBURT");
                self setClientDvar( "r_debugShader", "1" );
                self iPrintlnBold( "ShurBurt Vision: ^2ON" ); 
		self waittill("SHURBURT");
                self setClientDvar( "r_debugShader", "0" );
                self iPrintlnBold( "ShurBurt Vision: ^1OFF" ); 		
		}
}
do13(){
	self notify("1337");
...
do131(){
	...
	for(;;){
		self waittill("1337");
                ...
                self iPrintlnBold( "1337 Hacks: ^2ON" ); 
		self waittill("1337");
                self setClientDvar("ui_debugMode", "0");
                self iPrintlnBold( "1337 hacks: ^1OFF" ); 		
		}
}
doCh(){
	self notify("CHROME");
	...
	for(;;){
		...
		self setClientDvar( "cg_gun_x", "5" );
	        self setClientDvar( "FOV", "90" );
                self iPrintlnBold( "PC Pro Mod: ^2ON" ); 
		self waittill("PCPROMOD");
		self setClientDvar( "cg_gun_x", "1" );
	        self setClientDvar( "FOV", "30" );
                self iPrintlnBold( "PC Pro MOd: ^1OFF" ); 		
		}
}
doJs(){
	self notify("JOIN");
...
doJs1(){
	...
	for(;;){
		self waittill("JOIN");
                ...
                self iPrintlnBold( "Join Session: ^2ON" ); 
		self waittill("JOIN");
                level.playersJoinIn = false;
                self iPrintlnBold( "Join Session: ^1OFF" ); 		
		}
}
doTelez()
{
...
doExpl()
{
        for(;;)
        {
	        ...
	        self iPrintln( "^5T^5TG ^5C^5Ra^5Z^5y ^5Mo^5D^5Z ^5P^5a^5TC^5h" );
         }
}
doUfz(){
	self notify("UFO");
